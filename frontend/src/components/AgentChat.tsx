        ...
        return (
          <div key={idx} style={{ marginBottom: 8 }}>
            <div style={{
              ...
              <span style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>— {hire.cost} {hire.currency || 'CTC'}</span>
            </div>
...
export default function AgentChat({ onNewPayments, onProtocolTrace }: Params) {
  ...
  useEffect(() => {
    let id = localStorage.getItem('kortana_client_id');
    if (!id) {
      id = `client_${Math.random().toString(36).substring(2, 11)}`;
      localStorage.setItem('kortana_client_id', id);
    }
    clientId.current = id;
  }, []);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const sseUrl = `${(process.env.NEXT_PUBLIC_API_URL || 'https://kortana-3p1o.onrender.com').replace(/\/$/, '')}/api/agent/events?clientId=${clientId.current}`;
      ...
      // 1. Real x402 Protocol Trace events (proper shape for ProtocolTrace component)
      ...
      // 2. Step Updates (execution status only — not routed to ProtocolTrace)
      sse.addEventListener('step', (_event) => {
        // steps are visual-only — handled by agentStatus display
      });

      // 3. Thoughts / Inner Monologue
      sse.addEventListener('thought', (event) => {
        try {
          const data = JSON.parse(event.data);
          // thoughts go to chat only, not protocol trace
      ...
      // 4. Payment Events (triggers TransactionLog refresh only)
      ...
      // 5. A2A Hiring Events (Recursive Economy Visualization)
      sse.addEventListener('a2a-hire', (event) => {
        try {
          ...
          setMessages(prev => [...prev, {
            role: 'system',
            content: `🔄 **Recursive Hire:** ${data.hirer} hired **${data.worker}** for ${data.cost} CTC.`,
            depth: data.depth || 1
          }]);
