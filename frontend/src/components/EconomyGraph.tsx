...
const WORKER_AGENTS = [
  { id: 'seo-blog-agent', label: 'SEO Blog', slot: 0 },
  { id: 'twitter-thread-agent', label: 'Threads', slot: 1 },
  { id: 'tweet-update-agent', label: 'Tweets', slot: 2 },
  { id: 'infographic-agent', label: 'Infographic', slot: 3 },
  { id: 'marketing-campaign-agent', label: 'Campaign', slot: 4 },
  { id: 'ugc-video-agent', label: 'UGC Video', slot: 5 },
  { id: 'pitch-agent', label: 'Pitch', slot: 6 },
];
...
export default function EconomyGraph({ refreshTrigger = 0 }: { refreshTrigger?: number }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // User → Manager line
      drawEdge(ctx, userNode, managerNode, false, tick, '#6EE7B7');
      ...
      workerNodes.forEach(wn => {
        ...
        const color = isA2A ? '#a855f7' : '#6EE7B7'; // Purple for A2A, Orange for Standard
        drawEdge(ctx, managerNode, wn, isActive, tick, color);
      });

      // A2A recursive edges (campaign→pitch, seo-blog→twitter-thread)
  ...
  return (
    <div className="glass-panel" style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 20 }}>
          {[
            ...
            { label: 'Volume', value: `${stats.totalVolume} CTC`, color: '#6EE7B7' },
            { label: 'A2A Hires', value: stats.a2aCount, color: '#f59e0b' },
            { label: 'Agents', value: stats.activeAgents, color: '#6EE7B7' },
          ].map(s => (
      ...
      {/* Canvas */}
      <div style={{ position: 'relative', borderRadius: 0, overflow: 'hidden', border: '1px solid rgba(110, 231, 183, 0.15)', background: '#0A0A0A' }}>
        ...
        <div style={{
          ...
          {[
            { color: '#6EE7B7', label: 'User → Manager' },
            { color: '#6EE7B7', label: 'Manager → Worker' },
...
function agentIdFromEndpoint(endpoint: string): string {
  const map: Record<string, string> = {
    '/api/seo-blog': 'seo-blog-agent',
    '/api/twitter-thread': 'twitter-thread-agent',
    '/api/tweet-update': 'tweet-update-agent',
    '/api/infographic-prompt': 'infographic-agent',
    '/api/marketing-campaign': 'marketing-campaign-agent',
    '/api/ugc-video-prompt': 'ugc-video-agent',
    '/api/pitch': 'pitch-agent',
  };
