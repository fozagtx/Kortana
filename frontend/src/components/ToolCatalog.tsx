...
export default function ToolCatalog() {
  ...
  useEffect(() => {
    fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'https://kortana-3p1o.onrender.com').replace(/\/$/, '')}/api/tools`)
      .then(res => res.json())
      .then(data => {
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{
           ...
           <div>
             <h4 style={{ fontSize: '1rem', fontWeight: 800, margin: 0, textTransform: 'uppercase', letterSpacing: '-0.02em', color: '#FFFFFF' }}>{tool.name}</h4>
             <span className="mono" style={{ fontSize: '0.7rem', color: '#9CA3AF', fontWeight: 600 }}>{tool.category}</span>
           </div>
        </div>
      </div>

      <p style={{ fontSize: '0.85rem', color: '#9CA3AF', lineHeight: 1.5, marginBottom: 20, flex: 1, fontWeight: 500 }}>
        {tool.description}
      </p>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div className="mono" style={{ fontSize: '0.9rem', fontWeight: 800, color: '#FFFFFF' }}>
